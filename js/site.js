function generatingComponent(vardata, vargeodata){
    ...
    req_trends
        ...
        .compose([
            dc.lineChart(req_trends).group(groupfood_sec_req, 'Food Sec Requirement').colors(colors[5]).title(function (d) { return [dateFormatPretty(d.key),  "Food Sec Req: " + numberFormat(d.value) + ' Million US $'].join('\n'); }),
            dc.lineChart(req_trends).group(groupfunding, 'Funding').colors(colors[6]).title(function (d) { return [dateFormatPretty(d.key), "Funding: " + numberFormat(d.value) + ' Million US $'].join('\n'); }),
        ])
